  ...
  //	if( csctfLCTSource_.label() != "none" ){
  	  ...
  	  for(CSCCorrelatedLCTDigiCollection::DigiRangeIterator lctOfTrks = lctsOfTracks.begin();
 		...
 		( SubSectorLctTrk ? SubSectorLctTrk-1 : StationLctTrk );
	      ...
	      int sectorLUT = (*lctOfTrks).first.triggerSector()-1;
 		...
 		lclPhi = srLUTs_[FPGALctTrk][endcap][sectorLUT] -> localPhi(lctTrk->getStrip(),
 								 lctTrk->getPattern(),
 								 lctTrk->getQuality(),
 								 lctTrk->getBend() );
 		...
 		gblPhi = srLUTs_[FPGALctTrk][endcap][sectorLUT] -> globalPhiME(lclPhi.phi_local  ,
 								    lctTrk->getKeyWG(),
 								    CscIdLctTrk);
 		...
 		csctf_.trLctglobalPhi[nTrk-1][LctTrkId_] = gblPhi.global_phi;
   		...
   		csctf_.trLctglobalPhi[nTrk-1][LctTrkId_] = -999;
   	      ...
   	      try {
    		gblEta = srLUTs_[FPGALctTrk][endcap][sectorLUT] -> globalEtaME(lclPhi.phi_bend_local,
 								    lclPhi.phi_local     ,
 								    lctTrk->getKeyWG()   ,
 								    CscIdLctTrk);
...
//ALL csctf lcts
void L1Analysis::L1AnalysisCSCTF::SetLCTs(const edm::Handle<CSCCorrelatedLCTDigiCollection> corrlcts, CSCSectorReceiverLUT* srLUTs_[5][2][6])
{
	...
	int sectorLUT = (*corrLct).first.triggerSector()-1;
...
*/
	  ...
	  csctf_.lctstripNum.push_back(lct->getStrip());
          lclPhi = srLUTs_[FPGALct][endcap][sectorLUT] -> localPhi(lct->getStrip(),
                                                        lct->getPattern(),
                                                        lct->getQuality(),
                                                        lct->getBend() );
	...
	try {
	  ...
	  //std::cout << "CscIdLct: " << CscIdLct << std::endl;
          ...
          gblPhi = srLUTs_[FPGALct][endcap][sectorLUT] -> globalPhiME(lclPhi.phi_local  ,
                                                           lct->getKeyWG(),
                                                           CscIdLct);
	...
	try {
	  gblEta = srLUTs_[FPGALct][endcap][sectorLUT] -> globalEtaME(lclPhi.phi_bend_local,
							   lclPhi.phi_local     ,
							   lct->getKeyWG()   ,
							   CscIdLct);
